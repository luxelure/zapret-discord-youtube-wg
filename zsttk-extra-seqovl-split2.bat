...
--filter-udp=443 --hostlist="%LISTS%list-extra.txt" --hostlist="%LISTS%list-discord.txt" --hostlist="%LISTS%list-youtube.txt" --dpi-desync=fake,udplen --dpi-desync-repeats=6 --dpi-desync-udplen-increment=4 --dpi-desync-fake-quic="%BIN%quic_initial_www_google_com.bin" --new ^
--filter-tcp=443 --hostlist="%LISTS%list-extra.txt" --hostlist="%LISTS%list-discord.txt" --hostlist="%LISTS%list-youtube.txt" --dpi-desync=split2 --dpi-desync-split-pos=2 --dpi-desync-split-seqovl=681 --dpi-desync-split-seqovl-pattern="%BIN%tls_clienthello_www_google_com.bin"
