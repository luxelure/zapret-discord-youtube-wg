...
--filter-tcp=443 --ipset="%LISTS%ipset-cloudflare.txt" --dpi-desync=fake,multisplit --dpi-desync-split-pos=1,host+1,midsld-2,midsld,midsld+2,endhost-1,sniext+1 --dpi-desync-split-seqovl=681 --dpi-desync-split-seqovl-pattern="%BIN%tls_clienthello_www_google_com.bin" --dpi-desync-repeats=4 --dpi-desync-fooling=badseq,md5sig --dpi-desync-fake-tls="%BIN%zero_1.bin" --dpi-desync-fake-tls-mod=none --new ^
...
--filter-tcp=443 --hostlist="%LISTS%list-youtube.txt" --dpi-desync=fake,multisplit --dpi-desync-split-pos=1,host+1,midsld-2,midsld,midsld+2,endhost-1,sniext+1 --dpi-desync-split-seqovl=681 --dpi-desync-split-seqovl-pattern="%BIN%tls_clienthello_www_google_com.bin" --dpi-desync-repeats=4 --dpi-desync-fooling=md5sig --dpi-desync-fake-tls="%BIN%zero_1.bin" --dpi-desync-fake-tls-mod=none
